...
/*   Updated: 2018/12/19 20:25:27 by jbulant          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		draw_panel_slide(t_sdl *sdl, t_panel *pan)
{
	...
	draw_canvas_border(sdl, slide_box, CANVAS_INIT(0, 0), 0xababab);
}

static void		draw_panel_elems(t_sdl *sdl, t_panel *pan)
{
	t_canvas	e_anch;
	t_u32		tmp_min;
	t_bool		shade;
	...
	while (tmp_min < pan->view_max)
	{
		...
		draw_tex(sdl, pan->elem_tex[tmp_min], shade, e_anch);
		if (pan->hightlight && tmp_min == pan->cursor)
			draw_canvas_border(sdl, e_anch, CANVAS_INIT(0, 0), pan->border_col.rgba);
		tmp_min++;
		i++;
	}
}
...
{
	if (pan->draw_bg)
		draw_canvas_fill(sdl, pan->anchor, CANVAS_INIT(0, 0), pan->bg_col.rgba);
	if (pan->draw_border)
		draw_canvas_border(sdl, pan->anchor, CANVAS_INIT(0, 0),
				pan->border_col.rgba);
	draw_panel_elems(sdl, pan);
