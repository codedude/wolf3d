...
/*   Updated: 2019/01/09 17:14:21 by vparis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec2		allow_move(t_env *env, t_vec2 from, t_vec2 to, t_vec2 calc)
{
	...
	/*
	if ((map->data[(int)from.y][(int)(calc.x + offset.x)].id != 0
	...
	|| is_close_objects(env, VEC2_INIT(calc.x + offset.x, from.y)))
		calc.x = from.x;
	if ((map->data[(int)(calc.y + offset.y)][(int)from.x].id != 0
	...
	|| is_close_objects(env, VEC2_INIT(from.x, calc.y + offset.y)))
		calc.y = from.y;
	if ((map->data[(int)(calc.y + offset.y)][(int)(calc.x + offset.x)].id != 0
